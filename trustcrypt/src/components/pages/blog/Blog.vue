...
const categories = [
  ...
  {id: 2, name: 'Советы', categories: "adviсe"},
  {id: 3, name: 'Разработка', categories: "development"},
]

/*
Фильтрация, вывод товаров через axios
const blogs = ref([])

const axiosItems = async (categoryId: number) => {
  try {
    const {data} = await axios.get(`https://0f63305226082b32.mokky.dev/blogs/?categories=${categoryId}`)
    blogs.value = data
  } catch (err) {
    console.log(err)
  }
  ...
  <main class="main blog">
    ...
    <div class="main-wrapper">
      <div class="main-wrapper__item"
           v-for="blog in store.blogs" :key="blog.id" @click="goToBlogId(blog.id)">
